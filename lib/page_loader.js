    ...
    for (let page of inputComponents) {
            ...
            async goToComponent({ component, action = "main", data, type = "" }) {
                ...
                if (found_component) {
                    ...
                    try {
                        ...
                        return await runAction(found_component.actions[action], actionBinding, () => action_fn.bind(actionBinding)({ ctx: _this.ctx, data }));
                    }
                    catch (error) {
        ...
        if (!pageObject.open)
            pageObject.open = async function ({ ctx, data, action, clearChat, }) {
                ...
                const actionBinding = { ...binding, ctx };
                await runAction(pageObject.actions[act], actionBinding, () => action_fn.bind(actionBinding)({ ctx, data }));
            };
