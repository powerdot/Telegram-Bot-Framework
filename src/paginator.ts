...
function isComponentFile(file: string): boolean {
    if (declarationPattern.test(file)) return false;
    return componentExtensions.has(path.extname(file).toLowerCase());
}

function resolveDirectoryEntry(directory: string): string | undefined {
    try {
...
function loadComponent(componentPath: string): ComponentExport | undefined {
    ...
    return {
        list: function (componentType = "pages") {
            ...
            entries.forEach(function (entry) {
                ...
                let component_path: string | undefined;
                let id: string;
